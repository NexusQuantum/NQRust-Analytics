...
import { AnalyticsAIAdaptor } from '../analyticsAIAdaptor';
import {
...
describe('AnalyticsAIAdaptor', () => {
  const baseEndpoint = 'http://test-endpoint';
  let adaptor: AnalyticsAIAdaptor;

  beforeEach(() => {
    adaptor = new AnalyticsAIAdaptor({ analyticsAIBaseEndpoint: baseEndpoint });
    jest.clearAllMocks();
  });
