...
## :rocket: Get Started
1. Visit [How Analytics AI works?](https://docs.getanalytics.ai/oss/overview/how_analyticsai_works) to understand the architecture of Analytics AI
1. After you understand the architecture of Analytics AI, understand the scope of the services you want to contribute to.
...
### Example: Contributing to the [Analytics UI Service](#analytics-ui-service) and [Analytics Engine Service](#analytics-engine-service)
If you are contributing to both the [Analytics UI Service](#analytics-ui-service) and [Analytics Engine Service](#analytics-engine-service), you should comment out the `analytics-engine` service in the `docker/docker-compose-dev.yml` file (note that the UI service is already excluded from `docker/docker-compose-dev.yml`). Then, adjust the environment variables in your `.env` file to point to the services you have started manually. This will ensure that your local development environment correctly interfaces with the services you are working on.

1. Prepare your `.env` file: In the `AnalyticsAI/docker` folder, use the `.env.example` file as a template. Copy this file to create a `.env.local` file.
    ```sh
    # assuming the current directory is analytics-ui
    cd ../docker
    cp .env.example .env.local
    ```
2. Modify your `.env.local` file: Fill in the `OPENAI_API_KEY` with your OpenAI API keys before starting.
3. In the `AnalyticsAI/docker` folder, copy `config.example.yaml` to `config.yaml` for AI service configuration. Also change `http://analytics-ui:3000` to `http://host.docker.internal:3000` in `config.yaml`.
...
To develop a new data source connector, you'll need to modify both the front-end and back-end of the Analytics UI, in addition to the Analytics Engine.
...
#### Backend
1. Define the data source in `analytics-ui/src/apollo/server/dataSource.ts`
  - define the `toIbisConnectionInfo` and `sensitiveProps` methods

2. Modify the ibis adaptor in `analytics-ui/src/apollo/server/adaptors/ibisAdaptor.ts`
  - define an ibis connection info type for the new data source
  - set up the `dataSourceUrlMap` for the new data source

3. Modify the repository in `analytics-ui/src/apollo/server/repositories/projectRepository.ts`
  - define the analytics ui connection info type for the new data source 

4. Update the graphql schema in `analytics-ui/src/apollo/server/schema.ts` so that the new data source can be used in the UI 
  - add the new data source to the `DataSource` enum

5. Update the type definition in `analytics-ui/src/apollo/server/types/dataSource.ts`
...
2. Create the data source form template:
   - In `analytics-ui/src/components/pages/setup/dataSources`, add a new file named `${dataSource}Properties.tsx`
   - Implement the data source form template in this file

3. Set up the data source template:
   - Navigate to `analytics-ui/src/utils/dataSourceType.ts`
...
4. Update the data source list:
   - Add the new data source to the `DATA_SOURCES` enum in `analytics-ui/src/utils/enum/dataSources.ts`
   - Update relevant files in `analytics-ui/src/components/pages/setup/` to include the new data source
   - Ensure `analytics-ui/src/apollo/server/adaptors/ibisAdaptor.ts` handle the new data source
