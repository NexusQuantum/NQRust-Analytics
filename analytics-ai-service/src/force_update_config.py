...
# so we need to force update the engine names to the correct ones when we would like to use Analytics UI
            ...
            for pipe in doc.get("pipes", []):
                if "engine" in pipe:
                    if pipe["name"] == "sql_functions_retrieval":
                        pipe["engine"] = "analytics_ibis"
                    else:
                        pipe["engine"] = "analytics_ui"

    # Write back to the file
    with open("config.yaml", "w") as file:
        yaml.safe_dump_all(documents, file, default_flow_style=False)

    print("Successfully updated engine names to 'analytics_ui' in all pipelines")
