...
// then converts them to AnalyticsDataSource and Analytics MDL format
func DbtAutoConvert() {
	var opts struct {
		ProjectPath          string
		OutputDir            string
	...
	if opts.ProjectPath == "" {
		...
		pterm.Info.Println("Usage: analytics-launcher dbt-auto-convert --path /path/to/dbt/project --output /path/to/output")
		os.Exit(1)
	}
