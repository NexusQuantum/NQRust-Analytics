...
func validatePostgresDataSource(t *testing.T, ds *AnalyticsPostgresDataSource, expectedDB string) {
	t.Helper()
...
func TestFromDbtProfiles_Postgres(t *testing.T) {
	...
	ds, ok := dataSources[0].(*AnalyticsPostgresDataSource)
	if !ok {
		t.Fatalf("Expected AnalyticsPostgresDataSource, got %T", dataSources[0])
	}
...
func TestFromDbtProfiles_LocalFile(t *testing.T) {
	...
	ds, ok := dataSources[0].(*AnalyticsLocalFileDataSource)
	if !ok {
		t.Fatalf("Expected AnalyticsLocalFileDataSource, got %T", dataSources[0])
	}
...
func TestFromMssqlProfiles(t *testing.T) {
	...
	ds, ok := dataSources[0].(*AnalyticsMSSQLDataSource)
	if !ok {
		t.Fatalf("Expected AnalyticsMSSQLDataSource, got %T", dataSources[0])
	}
...
func TestFromDbtProfiles_BigQuery(t *testing.T) {
	...
	t.Run("service-account-json", func(t *testing.T) {
		...
		ds, ok := dataSources[0].(*AnalyticsBigQueryDataSource)
		if !ok {
			t.Fatalf("Expected AnalyticsBigQueryDataSource, got %T", dataSources[0])
		}
...
func TestPostgresDataSourceValidation(t *testing.T) {
	validDS := &AnalyticsPostgresDataSource{
		Host:     testHost,
		Port:     "5432",
		Database: "test",
		User:     "user",
...
func TestGetActiveDataSources(t *testing.T) {
	...
	ds := dataSources[0].(*AnalyticsPostgresDataSource)
...
func TestMapType(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			dataSource: &AnalyticsBigQueryDataSource{},
			sourceType: "INT64",
			want:       "integer",
		},
		{
		...
		},
		{
			...
			dataSource: &AnalyticsLocalFileDataSource{},
			sourceType: "INTEGER",
			want:       "integer",
		},
		{
		...
		},
		{
			...
			dataSource: &AnalyticsPostgresDataSource{},
